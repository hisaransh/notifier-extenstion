if (globalThis.__PR_BELL_JENKINS_MONITOR_LOADED__) {
  // Avoid double-registering observers when the script is injected manually and via manifest.
} else {
  globalThis.__PR_BELL_JENKINS_MONITOR_LOADED__ = true;
  ...
  (async () => {
    ...
    function evaluatePage() {
      ...
      const consoleOutput = getConsoleOutputNode();
    ...
    function getConsoleOutputNode() {
      const selectors = [
        'pre.console-output#out',
        'pre.console-output',
        '#out.console-output',
        'pre#out',
        '.console-output',
